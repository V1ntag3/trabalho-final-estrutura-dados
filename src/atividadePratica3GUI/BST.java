 ...
 * <p>
 * This implementation uses an (unbalanced) <em>binary search tree</em>. The
 ...
 * case, where <em>n</em> is the number of key-value pairs. The <em>size</em>
 * and <em>is-empty</em> operations take &Theta;(1) time. The keys method takes
 * &Theta;(<em>n</em>) time in the worst case. Construction takes &Theta;(1)
 * time.
 * <p>
...
public class BST<Key extends Comparable<Key>, Value> {
	...
	private int treeSize(Node no) {
		if (no == null)
			return 0;

		return 1 + treeSize(no.left) + treeSize(no.right);
	}
	...
	private void setDepthField(Node no, int i) {
		if (no != null) {
			no.depth = i;
			setDepthField(no.left, i + 1);
			setDepthField(no.right, i + 1);
		}
	}

	private int internalPathLength(Node no) {
		if (no == null)
			return 0;

		return no.depth + internalPathLength(no.left) + internalPathLength(no.right);
	}

	public ArrayList<String> showPre() {
		ArrayList<String> nodes = new ArrayList<>();
		return showPre(root, nodes);
	}

	private ArrayList<String> showPre(Node no, ArrayList<String> nodes) {
		if (no == null)
			return nodes;

		nodes.add((String) no.key);
		showPre(no.left, nodes);
		showPre(no.right, nodes);

		return nodes;
	}

	public ArrayList<String> showOrd() {
		ArrayList<String> nodes = new ArrayList<>();
		return showOrd(root, nodes);
	}

	private ArrayList<String> showOrd(Node no, ArrayList<String> nodes) {
		if (no == null)
			return nodes;

		showOrd(no.left, nodes);
		nodes.add((String) no.key);
		showOrd(no.right, nodes);

		return nodes;
	}

	public ArrayList<String> showPos() {
		ArrayList<String> nodes = new ArrayList<>();
		return showPos(root, nodes);
	}

	private ArrayList<String> showPos(Node no, ArrayList<String> nodes) {
		if (no == null)
			return nodes;

		showPos(no.left, nodes);
		showPos(no.right, nodes);
		nodes.add((String) no.key);
